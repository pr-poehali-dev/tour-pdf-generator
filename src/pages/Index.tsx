...
const days = [
  { d: '01', title: 'Прибытие и знакомство', stops: ['Трансфер из аэропорта', 'Заселение в отель «Lago Vista»', 'Приветственный ужин у озера'] },
  { d: '02', title: 'Горные тропы', stops: ['Подъём на смотровую площадку', 'Пикник на альпийском лугу', 'Свободное время в деревне'] },
  { d: '03', title: 'Озёрная одиссея', stops: ['Прогулка на катере', 'Дегустация местных вин', 'Закатная фотосессия'] },
  { d: '04', title: 'Старый город', stops: ['Экскурсия по историческому центру', 'Мастер-класс местной кухни', 'Прощальный гала-ужин'] },
];
          ...
          <div className="space-y-4">
            {days.map((day) => (
              <div key={day.d} className="bg-card rounded-3xl p-8 shadow-sm border border-sand flex flex-col md:flex-row gap-6">
                <div className="font-display text-6xl font-semibold text-coral/30 leading-none">{day.d}</div>
                <div className="flex-1">
                  <h3 className="font-display text-3xl font-semibold mb-4">{day.title}</h3>
                  <div className="space-y-2">
                    {day.stops.map((s) => (
                      <div key={s} className="flex items-center gap-3 text-ink/75">
                        <Icon name="MapPin" size={16} className="text-ocean shrink-0" />
                        <span>{s}</span>
                      </div>
                    ))}
                  </div>
